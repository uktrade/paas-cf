resource "datadog_monitor" "router" {
  name = "${data.null_data_source.datadog.env} router hosts"
  type = "service check"
  ...
  no_data_timeframe = "2"
  query = "'datadog.agent.up'.over('environment:${data.null_data_source.datadog.env}','job:router').by('*').last(1).pct_by_status()"
  ...
  tags {
    "environment" = "${data.null_data_source.datadog.env}"
    "job" = "router"
  }
}
