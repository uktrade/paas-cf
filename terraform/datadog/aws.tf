...
resource "datadog_monitor" "ec2-es-cpu-utilisation" {
  name                = "${format("%s EC2 ElasticSearch high CPU utilisation", var.env)}"
  type                = "metric alert"
  query               = "${format("avg(last_1h):avg:aws.ec2.cpuutilization{deploy_env:%s,job:elasticsearch_master} by {bosh-index} > 98", var.env)}"
  message             = "{{bosh-job.name}}/{{bosh-index.name}} CPU utilisation has been over {{#is_warning}}{{warn_threshold}}{{/is_warning}}{{#is_alert}}{{threshold}}{{/is_alert}}% for 1h"
  notify_no_data      = false
  require_full_window = false
