...
resource "datadog_monitor" "continuous-smoketests" {
  ...
  type               = "query alert"
  message            = "${format("Continuous smoketests too slow: {{value}} ms. Check concourse VM health. @govpaas-alerting-%s@digital.cabinet-office.gov.uk  {{#is_no_data}}The continuous smoke tests  have not reported any metrics for a while. Check concourse status. @pagerduty-Datadog_no_data {{/is_no_data}}", var.aws_account)}"
