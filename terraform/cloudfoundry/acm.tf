...
resource "aws_acm_certificate" "istio_apps" {
  domain_name               = "*.istio.${var.apps_dns_zone_name}"
  subject_alternative_names = ["istio.${var.apps_dns_zone_name}"]
  validation_method         = "DNS"
}

resource "aws_route53_record" "istio_apps_cert_validation" {
  name    = "${aws_acm_certificate.istio_apps.domain_validation_options.0.resource_record_name}"
  type    = "${aws_acm_certificate.istio_apps.domain_validation_options.0.resource_record_type}"
  zone_id = "${var.apps_dns_zone_id}"
  records = ["${aws_acm_certificate.istio_apps.domain_validation_options.0.resource_record_value}"]
  ttl     = 60
}

resource "aws_acm_certificate_validation" "istio_apps" {
  certificate_arn = "${aws_acm_certificate.istio_apps.arn}"

  validation_record_fqdns = [
    "${aws_route53_record.istio_apps_cert_validation.fqdn}",
  ]
}
