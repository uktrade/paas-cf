...
var _ = Describe("HTTP edge cases", func() {
	...
	It("should accept large response body sizes", func() {
		for responsekB := 1; responsekB <= 200; responsekB += 10 {
			By(fmt.Sprintf("response body size of %d kB", responsekB))

			appUri := helpers.AppUri(appName, fmt.Sprintf("/big-body?size=%d", responsekB), testConfig)
			response, err := httpClient.Get(appUri)
			Expect(err).NotTo(HaveOccurred())
			bodyBytes, err := ioutil.ReadAll(response.Body)
			Expect(err).NotTo(HaveOccurred())
			Expect(bodyBytes).To(HaveLen(responsekB * int(KILOBYTE)))
		}
	})
