...
routing_api_pass=${TF_VAR_external_routing_api_database_password:?}
...
psql_adm -d postgres -c "SELECT rolname FROM pg_roles WHERE rolname = 'routing_api'" \
  | grep -q 'routing_api' || psql_adm -d postgres -c "CREATE USER routing_api WITH ROLE dbadmin"
...
psql_adm -d postgres -c "ALTER USER routing_api WITH PASSWORD '${routing_api_pass}'"

for db in api uaa bbs locket network_connectivity network_policy routing_api; do
