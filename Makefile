...
upload-all-secrets: upload-google-oauth-secrets upload-microsoft-identity-secrets upload-notify-secrets upload-aiven-secrets upload-logit-secrets upload-pagerduty-secrets
...
.PHONY: upload-microsoft-identity-secrets
upload-microsoft-identity-secrets: check-env ## Decrypt and upload Microsoft Identity credentials to S3
	$(eval export OAUTH_PASSWORD_STORE_DIR?=${HOME}/.paas-pass)
	$(if ${MAKEFILE_ENV_TARGET},,$(error Must set MAKEFILE_ENV_TARGET))
	$(if ${OAUTH_PASSWORD_STORE_DIR},,$(error Must pass OAUTH_PASSWORD_STORE_DIR=<path_to_password_store>))
	$(if $(wildcard ${OAUTH_PASSWORD_STORE_DIR}),,$(error Password store ${OAUTH_PASSWORD_STORE_DIR} does not exist))
	@scripts/upload-microsoft-identity-secrets.sh
